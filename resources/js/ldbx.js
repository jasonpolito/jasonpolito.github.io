...
export class LDBX {
    constructor(tableOrConfig = null, showLog = true, softDelete = true) {
        if (typeof tableOrConfig === "object") {
            if (tableOrConfig.table) this.tableName = tableOrConfig.table;
                ...
                this.console(
                    "Table name not defined, plese make object with name 'table' if you using object config params!"
                );

            if (tableOrConfig.softDelete)
                this.softDelete = tableOrConfig.softDelete;
  ...
  */
    fixStructure(fillableColumns) {
        ...
        console.info(
            `Success updating structure [${this.tableName}]`,
            fillableColumns,
            data
        );
    }
    ...
    fetch(tableName, criteria = [], tag = "") {
        ...
        if (this.orderCondition.length) {
            for (let order of this.orderCondition) {
                if (order.dateFormat)
                    data = this.sortByDateFormat(data, order.key, order.type);
        ...
        this.console(
            `${tag} Query ${data.length} of '${tableName}' Execution time: ${
                end - start
            } ms`,
            "info"
        );
    ...
    operator(operator, value, valueComparison) {
        ...
        if (
            Array.isArray(valueComparison) &&
            operator === "IN" &&
            valueComparison.includes(value)
        )
            res = true; // IN
        if (
            Array.isArray(valueComparison) &&
    ...
    query(data, criteria = []) {
        ...
        for (let val of data) {
            ...
            for (let where of criteria) {
                ...
                if (this.operator(where.operator, colVal, value))
                    match = match + 1;
            }
            if (match === countWhere) {
                // if (this.softDelete) {
    ...
    where(col, oprOrVal, value) {
        ...
        if (criteria.operator.toLowerCase() === "n")
            criteria = { key: col, operator: "N", value: null };
        if (criteria.operator.toLowerCase() === "nn")
            criteria = { key: col, operator: "NN", value: null };
        if (criteria.operator.toLowerCase() === "in")
            criteria = { key: col, operator: "IN", value: value };
        if (criteria.operator.toLowerCase() === "nin")
            criteria = { key: col, operator: "NIN", value: value };
    ...
    collectRelation(data) {
        return data.map((obj) => {
            if (this.belongsToRelation.length)
                obj = this.getRelation(obj, "belongsTo");
            if (this.hasManyRelation.length)
                obj = this.getRelation(obj, "hasMany");
            return obj;
        });
    }
    ...
    getRelation(obj, type = "belongsTo") {
        ...
        this[relationMapName].map((rel) => {
            let objRel = this.fetch(rel.tableName, [
                {
                    key: rel.targetKey,
                    operator: "=",
                    value: obj[rel.foreignKey],
                },
            ]);
    ...
    save(data) {
        ...
        const db = table
            ? [...table, { ...data, id, created_at, updated_at, deleted_at }]
            : [{ ...data, id, created_at, updated_at, deleted_at }]; // merging
    ...
    update(data) {
        ...
        if (index >= 0) {
            ...
            this.console(
                `update:: index of id [${data.id}] not found in data "${this.tableName}" `,
                "danger"
            );
            return false;
        }
    ...
    delete(id, force = false) {
        ...
        if (index >= 0) {
            ...
            this.console(
                `delete: index of id [${id}] not found in data "${this.tableName}" `,
                "danger"
            );
            return false;
        }
